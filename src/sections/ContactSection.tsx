...
export const ContactSection = () => {
  ...
  useEffect(() => {
    ...
    const redirectUrl = `${window.location.origin}/gracias.html`;
    nextUrlRef.current.value = redirectUrl;
  }, []);
      ...
      <div className="mx-auto max-w-6xl px-6">
        <FadeInWhenVisible className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-3 text-sm text-slate-600 md:text-base">
            Cuéntanos qué necesitas proteger y cómo te podemos apoyar. Te
            respondemos con una propuesta clara y sin letra pequeña.
          </p>
        </FadeInWhenVisible>
          ...
          <FadeInWhenVisible className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white px-6 py-7 shadow-md md:px-8 md:py-8">
            ...
            >
              <input
                type="hidden"
                name="_subject"
                value="Nuevo contacto ABP Seguros"
              />
              <input
                ref={nextUrlRef}
                type="hidden"
              ...
              <label className="md:col-span-2 space-y-1.5">
                ...
                <textarea
                  name="mensaje"
                  placeholder="Ejemplo: Necesito cotización para seguros de vida para mi familia y póliza de riesgos laborales para mi empresa con 10 empleados."
                  rows={4}
                  className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-700 shadow-sm transition focus:border-abp-blue focus:outline-none focus:ring-2 focus:ring-abp-blue/20"
                />
                <p className="text-[0.7rem] text-slate-500">
                  Cuéntanos qué quieres proteger, si ya tienes alguna póliza y
                  qué te gustaría mejorar o revisar.
                </p>
              </label>
